$('.infoleft').each(function(){
    let tapDiv=$(this);
    let anchors=tapDiv.find('button a');
    let panelDivs=tapDiv.find('.infotab');
    let lastAnchor;
    let lastPanel;
...
// const targetLink1=document.querySelectorAll('.infoleft>button a');
// const tabContent1=document.querySelectorAll('.position-box ul');
...
// for(let i=0; i<targetLink1.length; i++){
...
//             for(let j=0; j<targetLink1.length; j++){
//                 targetLink1[j].classList.remove('active');
//                 e.target.classList.add('active');
                
//             }
//             for(let k=0; k<tabContent1.length; k++){
//                 tabContent1[k].style.display='none'
//             }
